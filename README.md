...
- **코드 기반 매핑** (명칭 변동에 강함)
  ...
  - 상품코드 → 대표=한익스상품명 + **구분(대월/프리미엄)** (전 채널 공통 마스터)
  - 코드 미등록 시 화주상품명으로 자동 폴백(`product_names.json`) → 프리미엄 자동 분류
- **상품명 표기**: 부착양식 라벨=화주상품명(원본), TOTAL 표지=한익스상품명(대표)
...
- 라벨은 창고 분류에 맞춰 **구분 → 거점센터 → 대표상품 → 점포명** 순 정렬
